...
% -- Versions ------------------------------------------------------------------
% Declare the per-exam letter labels. A normal build compiles every version
% into one combined PDF, then the Sublime builder slices <base>_A.pdf,
% <base>_B.pdf, ... out of it afterward.
\versions{A, B, C}
...
% Multiple-choice problems go in a \begin{mcproblems} ... \end{mcproblems}
% section (auto-headed "Part N — Multiple Choice"): choices shift left, an
% "Answer:" line is added, and in solution builds the worked solution is set
% beside the options.  Author them in the bank with \begin{choices} (\cchoice
% marks the answer; \fchoice[i] forces an option; [choose=m] presents m of them).
