...
% The exam-class statements (exam*/ex*/ques*/prob*) carry inline lead-ins
% \solution / \answer (alias \ans) / \pf for a worked solution in line with the
% statement. They are ALWAYS visible, so use them for a reference example like
% the one below — for a HIDDEN answer key, use the gated \begin{solution} box
% inside a problem/question (as the bank entries above do).
\begin{exam*}
	(Reference) Evaluate $2 \cdot 3^2 - 4$.
	\solution $2 \cdot 9 - 4 = 14$.
\end{exam*}
...
% \extracredit reserves its own answer space (a trailing \workbox, suppressed in
% key builds), so — unlike the bare \question items above — it needs no manual
% \vspace after it.  An optional second argument sets the space: \extracredit[2][2].
